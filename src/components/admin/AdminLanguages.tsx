import React, { useState } from 'react';
import { Language } from '../../types';
import { portfolioService } from '../../lib/portfolioService';
import { Dialog } from '../ui/Dialog';
import { Plus, Pencil, Trash2, Languages as LangIcon } from 'lucide-react';

interface AdminLanguagesProps {
  languages: Language[];
  onLanguagesUpdated: (languages: Language[]) => void;
}

export function AdminLanguages({
  languages,
  onLanguagesUpdated,
}: AdminLanguagesProps) {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [editingLang, setEditingLang] = useState<Language | null>(null);
  const [formData, setFormData] = useState({
    name: '',
    level: 'Native',
    sort_order: 0,
  });
  const [isSubmitting, setIsSubmitting] = useState(false);

  const openAdd = () => {
    setEditingLang(null);
    setFormData({
      name: '',
      level: 'Native',
      sort_order: languages.length + 1,
    });
    setIsDialogOpen(true);
  };

  const openEdit = (lang: Language) => {
    setEditingLang(lang);
    setFormData({
      name: lang.name,
      level: lang.level,
      sort_order: lang.sort_order || 1,
    });
    setIsDialogOpen(true);
  };

  const handleDelete = async (id: string, name: string) => {
    if (confirm(`Hapus bahasa "${name}"?`)) {
      try {
        await portfolioService.deleteLanguage(id);
        onLanguagesUpdated(languages.filter((l) => l.id !== id));
      } catch (err: any) {
        alert('Gagal menghapus: ' + err.message);
      }
    }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSubmitting(true);
    try {
      if (editingLang) {
        const updated = await portfolioService.updateLanguage(editingLang.id, formData);
        onLanguagesUpdated(
          languages.map((l) => (l.id === editingLang.id ? updated : l))
        );
      } else {
        const created = await portfolioService.addLanguage(formData);
        onLanguagesUpdated([...languages, created]);
      }
      setIsDialogOpen(false);
    } catch (err: any) {
      alert('Gagal menyimpan: ' + err.message);
    } finally {
      setIsSubmitting(false);
    }
  };

  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-2xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Kelola Kemampuan Bahasa</h2>
            <p className="text-sm text-slate-500 mt-1">
              Sesuai PRD 4.6, bahasa ditampilkan sebagai teks sederhana format: {'{Nama Bahasa} — {Level}'}.
            </p>
          </div>
          <button
            onClick={openAdd}
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-semibold transition-colors shadow-2xs cursor-pointer self-start sm:self-auto"
          >
            <Plus className="w-4 h-4" />
            <span>Tambah Bahasa</span>
          </button>
        </div>

        {/* Table */}
        <div className="overflow-x-auto mt-4">
          <table className="w-full text-left border-collapse text-sm">
            <thead>
              <tr className="border-b border-slate-200 text-slate-500 text-xs font-semibold uppercase tracking-wider bg-slate-50/50">
                <th className="py-3 px-4">Nama Bahasa</th>
                <th className="py-3 px-4">Tingkat Kemampuan (Level)</th>
                <th className="py-3 px-4 text-center">Urutan</th>
                <th className="py-3 px-4 text-right">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {languages.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-slate-400">
                    Belum ada data bahasa.
                  </td>
                </tr>
              ) : (
                languages.map((lang) => (
                  <tr key={lang.id} className="hover:bg-slate-50/60 transition-colors">
                    <td className="py-3.5 px-4 font-semibold text-slate-900 flex items-center gap-2">
                      <LangIcon className="w-4 h-4 text-blue-600" />
                      <span>{lang.name}</span>
                    </td>
                    <td className="py-3.5 px-4 text-slate-700">
                      {lang.level}
                    </td>
                    <td className="py-3.5 px-4 text-center font-mono text-xs text-slate-500">
                      {lang.sort_order || 1}
                    </td>
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => openEdit(lang)}
                          className="p-1.5 rounded-md text-slate-600 hover:text-blue-600 hover:bg-blue-50 transition-colors"
                        >
                          <Pencil className="w-4 h-4" />
                        </button>
                        <button
                          onClick={() => handleDelete(lang.id, lang.name)}
                          className="p-1.5 rounded-md text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>
      </div>

      <Dialog
        isOpen={isDialogOpen}
        onClose={() => setIsDialogOpen(false)}
        title={editingLang ? 'Edit Bahasa' : 'Tambah Bahasa Baru'}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold uppercase text-slate-600 tracking-wider mb-1">
              Nama Bahasa *
            </label>
            <input
              type="text"
              required
              value={formData.name}
              onChange={(e) => setFormData({ ...formData, name: e.target.value })}
              placeholder="Contoh: Bahasa Indonesia / Bahasa Inggris"
              className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>

          <div>
            <label className="block text-xs font-semibold uppercase text-slate-600 tracking-wider mb-1">
              Tingkat Kemampuan (Level) *
            </label>
            <input
              type="text"
              required
              value={formData.level}
              onChange={(e) => setFormData({ ...formData, level: e.target.value })}
              placeholder="Contoh: Native / Intermediate / Fluent / Basic"
              className="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <p className="text-2xs text-slate-500 mt-1">
              Pilihan umum: Native, Fluent, Professional Working, Intermediate, Conversational.
            </p>
          </div>

          <div className="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
            <button
              type="button"
              onClick={() => setIsDialogOpen(false)}
              className="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-lg"
            >
              Batal
            </button>
            <button
              type="submit"
              disabled={isSubmitting}
              className="px-5 py-2 text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white rounded-lg disabled:opacity-50"
            >
              {isSubmitting ? 'Menyimpan...' : 'Simpan'}
            </button>
          </div>
        </form>
      </Dialog>
    </div>
  );
}
